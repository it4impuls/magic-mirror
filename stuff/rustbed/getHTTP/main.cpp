...
void print(const shared_ptr< Response >& response)
{
    ...
    fprintf(stderr, "Body:           %.*s\n", static_cast<unsigned int>(response->get_body().size()), response->get_body().data());
}

int main(const int, const char**)
{
    auto request = make_shared< Request >(Uri("http://worldtimeapi.org/api/timezone/Europe/Berlin"));
